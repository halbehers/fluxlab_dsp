    ...
    _fxGrid(audioProcessor, audioProcessor.audioEngine)
{
    ...
    getLayout().init({ 3, 12, 40, 5 }, { 1, 2, 11, 3, 3, 11, 3, 17 });

    getLayout().addComponent(_title, 0, 0, 8, 1);
    getLayout().addComponent(_audioLevels, 1, 0, 1, 1);
    getLayout().addComponent(_audioSpectrum, 1, 1, 7, 1);
    getLayout().addComponent(_tempoSection, 3, 2, 1, 1);
    getLayout().addComponent(_crossfader, 3, 5, 1, 1);
    //getLayout().addComponent(_tabs, 3, 5, 1, 1);
    getLayout().addComponent(_fxGrid, 2, 0, 8, 1);
